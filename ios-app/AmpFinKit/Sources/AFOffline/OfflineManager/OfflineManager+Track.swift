...
internal extension OfflineManager {
    func download(track: Track, context: ModelContext) {
        ...
        Task.detached {
            do {
                let downloadTask = try await DownloadManager.shared.download(trackId: trackId)
                let context = ModelContext(PersistenceManager.shared.modelContainer)
                if let offlineTrack = try? self.offlineTrack(trackId: trackId, context: context) {
                    offlineTrack.downloadId = downloadTask.taskIdentifier
                    try? context.save()
                }
                downloadTask.resume()
            } catch {
                DownloadManager.shared.logger.fault("Failed to resolve stream URL for \(trackId): \(error.localizedDescription)")
                let context = ModelContext(PersistenceManager.shared.modelContainer)
                if let offlineTrack = try? self.offlineTrack(trackId: trackId, context: context) {
                    context.delete(offlineTrack)
                    try? context.save()
                }
                NotificationCenter.default.post(name: OfflineManager.itemDownloadStatusChanged, object: trackId)
            }
            await self.updateLyrics(trackId: trackId)
        }
    ...
    func update(trackId: String) throws {
        ...
        Task.detached {
            do {
                ...
                if let offlineTrack = try? self.offlineTrack(trackId: trackId, context: ctx) {
                    offlineTrack.downloadId = downloadTask.taskIdentifier
                    try? ctx.save()
                }
                downloadTask.resume()
