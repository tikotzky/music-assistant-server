...
internal extension OfflineManager {
    func create(playlist: Playlist, tracks: [Track], context: ModelContext) throws -> OfflinePlaylist {
        ...
        for track in tracks {
            ...
            guard !seenAlbumIds.contains(albumId), !DownloadManager.shared.coverDownloaded(parentId: albumId), let cover = track.cover else {
                seenAlbumIds.insert(albumId)
                continue
            }
            seenAlbumIds.insert(albumId)
            Task.detached {
                try await DownloadManager.shared.downloadCover(parentId: albumId, cover: cover)
            }
        }
