...
internal extension LocalAudioEndpoint {
    func startPlayback(tracks: [Track], startIndex: Int, shuffle: Bool) {
        ...
        // Set up audio session before updating now playing metadata
        #if !os(macOS)
        ...
        // Resolve the first track's stream URL, then start playback
        Task { [weak self] in
            guard let self, let track = self.nowPlaying else { return }
            let item = await self.avPlayerItemAsync(track: track)
            await MainActor.run {
                // Check if populateAVPlayerQueue already inserted this track
                guard !self.avPlayerQueue.contains(track.id) else {
                    self.playing = true
                    return
                }
    ...
    var mediaInfo: Track.MediaInfo? {
        get async {
            if let itemId = nowPlaying?.id {
                ...
                if !serverBitrateLikelyToBeFalse, var mediaInfo = try? await TwentyFourSixClient.shared.mediaInfo(trackId: itemId) {
